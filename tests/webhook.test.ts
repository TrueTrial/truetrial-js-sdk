...
import { OrderStatus, WebhookEvent } from '../src/types/enums.js';
...
describe('delivery.failed event', () => {
  it('exposes delivery.failed in the WebhookEvent enum', () => {
    expect(WebhookEvent.DeliveryFailed).toBe('delivery.failed');
  });

  it('exposes delivery_failed in the OrderStatus enum', () => {
    expect(OrderStatus.DeliveryFailed).toBe('delivery_failed');
  });

  it('parses a delivery.failed webhook payload', () => {
    const secret = 'whsec_test';
    const payload = buildPayload({
      event: 'delivery.failed',
      data: { orderId: '01ABC', externalOrderId: 'ORD-1', failedAt: '2026-05-19T10:00:00Z' },
    });
    const signature = sign(payload, secret);

    const result = parseWebhook(payload, signature, secret);
    expect(result.event).toBe('delivery.failed');
  });
});
